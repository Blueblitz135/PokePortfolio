...
def test_calculation_summary_uses_highest_id_for_equal_snapshot_times() -> None:
    observed_at = datetime(2026, 2, 1, tzinfo=timezone.utc)
    asset = Asset(
        asset_type=AssetType.SEALED_PRODUCT,
        display_name="Booster Box",
...
def test_calculation_summary_has_no_roi_when_total_cost_is_zero() -> None:
    asset = Asset(
        asset_type=AssetType.SEALED_PRODUCT,
        display_name="Gifted Booster Box",
        purchase_lots=[
            PurchaseLot(
                purchase_date=date(2025, 8, 1),
                quantity=1,
        ...
        price_snapshots=[
            PriceSnapshot(
                market_price_per_unit=Decimal("100.00"),
                observed_at=datetime(2026, 2, 1, tzinfo=timezone.utc),
            )
        ],
    )
    ...
    assert summary.profit_loss == Decimal("100.00")
    assert summary.roi_percent is None
